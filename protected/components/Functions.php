...
class Functions
{
    ...
    public static function makeTextCard($str)
    {
        $tmp = Functions::locDau($str);
        $tmp = str_replace("-", " ", $tmp);
        return strtoupper($tmp);
...
$autoCreate = isset($mConfig['autoCreate']) ? $mConfig['autoCreate'] : false;
        ...
        if($collate === 'utf8_unicode_ci' || $collate === "datetime") $collate = '';
        foreach ($database as $table => $conf) {
            $className = ucfirst($table) . "Model";
            $label = "";
            $var_arr = "id integer NOT NULL";/*AUTO_INCREMENT*/
            $select_name = "id";
            foreach ($conf as $key => $value) {
                ...
                if (strtoupper($type) == "RELATION") {
                    ...
                    if($collate === ''){
                        $var_arr .= ",$key ".((strtoupper($type)=='STRING')?'text':$type)." $notnull"; /*COLLATE */
                    }else{
                        $var_arr .= ",$key ".((strtoupper($type)=='STRING')?'text':$type)." $notnull  COLLATE $collate"; /*COLLATE */
                    }
                }
                $label .= "'$key'=>array(
                        'name'=>'$tagName',
            ...
            if(($tableLog != "")){
                $var_arr .= ", create_by integer DEFAULT NULL, CONSTRAINT ".$table."_create_on FOREIGN KEY (create_by) REFERENCES $tableLog (id) ON DELETE SET NULL";
                $var_arr .= ", modify_by integer DEFAULT NULL, CONSTRAINT ".$table."_modify_by FOREIGN KEY (modify_by) REFERENCES $tableLog (id) ON DELETE SET NULL";
                $var_arr .= ", modify_on date";
                $var_arr .= ", create_on date";
            }
...
//        Yii::app()->params['skype'] = $conf->find("code = 'SKYPE'")['value'];
//        Yii::app()->params['facebook'] = $conf->find("code = 'FACEBOOK'")['value'];
//        Yii::app()->params['contact'] = $conf->find("code = 'CONTACT'")['value'];
//        Yii::app()->params['twitter'] = $conf->find("code = 'TWITTER'")['value'];
    }
