...
$filenameSum = Yii::getPathOfAlias('webroot') . '/assets/js/tcdb/data/earthquakes.geojson';
$jsonSum = json_decode(file_get_contents($filenameSum), true);
$dataSum = count($jsonSum['features']);

$filename0 = Yii::getPathOfAlias('webroot') . '/assets/js/tcdb/data/bridge_khongthongtin.geojson';
$json0 = json_decode(file_get_contents($filename0), true);
$data0 = count($json0['features']);

$filenameVG = Yii::getPathOfAlias('webroot') . '/assets/js/tcdb/data/bridge_rattot.geojson';
$jsonVG = json_decode(file_get_contents($filenameVG), true);
$dataVG = count($jsonVG['features']);

$filenameG = Yii::getPathOfAlias('webroot') . '/assets/js/tcdb/data/bridge_tot.geojson';
$jsonG = json_decode(file_get_contents($filenameG), true);
$dataG = count($jsonG['features']);

$filenameT = Yii::getPathOfAlias('webroot') . '/assets/js/tcdb/data/bridge_trungbinh.geojson';
$jsonT = json_decode(file_get_contents($filenameT), true);
$dataT = count($jsonT['features']);

$filenameB = Yii::getPathOfAlias('webroot') . '/assets/js/tcdb/data/bridge_yeu.geojson';
$jsonB = json_decode(file_get_contents($filenameB), true);
$dataB = count($jsonB['features']);

$filenameVB = Yii::getPathOfAlias('webroot') . '/assets/js/tcdb/data/bridge_ratyeu.geojson';
$jsonVB = json_decode(file_get_contents($filenameVB), true);
$dataVB = count($jsonVB['features']);
?>
<div id="Map-content">
<section id="header">
    ...
    <div class="panel-group">
        <div class="panel panel-default">
            ...
            <div id="collapse-ly-trinh" class="panel-collapse collapse">
                <div class="option-style data-font-size">
                    <div class="option-checkbox text-center">
                        <input type='checkbox' name='thing' value='valuable' id="ly-trinh"/><label
                            for="ly-trinh"></label>
                    </div>
                    <div class="option-text data-font-size">
                        <span>Lý trình</span>
...
<!--</div>-->
<script src="<?php echo Yii::app()->getBaseUrl(true) ?>/assets/js/tcdb/SourceBridge.js"></script>
<script src="<?php echo Yii::app()->getBaseUrl(true) ?>/assets/js/tcdb/toolBar.js"></script>
<script src="<?php echo Yii::app()->getBaseUrl(true) ?>/assets/js/tcdb/Search.js"></script>
<script src="<?php echo Yii::app()->getBaseUrl(true) ?>/assets/js/tcdb/scale-control.js"></script>
<script src="<?php echo Yii::app()->getBaseUrl(true) ?>/assets/js/tcdb/buttonEvent.js"></script>
<script src="<?php echo Yii::app()->getBaseUrl(true) ?>/assets/js/tcdb/route.js"></script>
<script src="<?php echo Yii::app()->getBaseUrl(true) ?>/assets/js/tcdb/CreHTML.js"></script>
<script src="<?php echo Yii::app()->getBaseUrl(true) ?>/assets/js/tcdb/marker.js"></script>
<script src="<?php echo Yii::app()->getBaseUrl(true) ?>/assets/js/tcdb/TouchEvt.js"></script>
<script>
...
//            $.ajax({
//                url: "http://123.30.59.161:8000/api/bridge/GeoJson?type=B",
//                method: "GET",
//                headers: {
...
//            })
    ...
    //    map.on('load', function () {
    //        if (map.getSource("bridge_0")) {
    //            map.removeSource("bridge_0");
    //        }
    //        if (map.getLayer("clusters-0") && map.getLayer("unclustered-point-0")) {
    //            map.removeLayer("clusters-0");
    //            map.removeLayer("unclustered-point-0");
    //        }
    //        map.addSource("bridge_0", Source_0);
    //        map.addLayer({
    //            id: "clusters-0",
    //            type: "symbol",
    //            source: "bridge_0",
    //            layout : {
    //                "icon-image": "briegd_test_15"
    //            }
    //        });
    //
    //        map.addLayer({
    ...
    //            var popup_0 = new mapboxgl.Popup({
    //                closeButton: false,
    //                closeOnClick: true
    //            });
    //            var features_0 = map.queryRenderedFeatures(e.point, {
    //                layers: ['unclustered-point-0']
    //            });
    //            if (!features_0.length) {
    //                map.getCanvas().style.cursor = '';
    //                popup_0.remove();
    //                return
    //            }
    //            map.getCanvas().style.cursor = 'pointer';
    //            var feat_0 = features_0[0];
    //            console.log('Không thông tin', feat_0);
    //            popup_0.setLngLat(feat_0.geometry.coordinates)
    //                .setText(feat_0.properties.brist + ' (' + feat_0.properties.name + ')')
    //                .addTo(map);
    //        });
    //    });
    /*Cầu địa phương*/

    //,{"type":"Feature","geometry":{"type":"Point","coordinates":[104.121094,22.350532]},"properties":{"bridgeid":10394,"constructi":"","name":"Suối Trát","chainage":"20 Km 50","prov":"LO","city":"","longitude":104.121094,"latitude":22.350532,"pitch":0,"roadtype":"N","crossobj":"SOI","buildyr":0,"owner":"Cục QLĐB I","unit":"Chi cục QLĐB I.3","signtype":"Biển 115","pltonnage":"25","roadno":"QL04E","uplaneno":2,"unlaneno":0,"detonnage":"Không biết","brist":"Yếu","exploreyr":1993,"length":35,"width":6.5,"mainpierno":1,"pierno":0,"longestpie":19.5,"ttlpierlen":19.5,"drivesize":4,"walksize":1,"grosssize":6.5,"skewangle":0,"bciav":100,"bcicrit":115,"images":"10394_Picture_1364.jpg;10394_Cáº¦USUá»ITRÃT1.jpg;10394_Suá»itrÃ¡t1.jpg;10394_Suá»itrÃ¡t2.jpg;10394_Suá»itrÃ¡t3.jpg;10394_Suá»itrÃ¡4.jpg;10394_Suá»itrÃ¡5.jpg;10394_Suá»itrÃ¡t6.jpg;10394_Suá»itrÃ¡7.jpg;10","mgrname":"Cục Quản lý đường bộ I"}}
    //});
